...
#Create two additional variables: "logical" in which "reasoning" observations are coded as 1 = logical reasoning and 0 = other reasoning, and "age_category".

pp_data <- raw_pp_data %>% 
  mutate(logical = ifelse(reasoning==4, 1, 0)) %>% 
  mutate(age_category = ifelse(age<3, 2, ifelse(age>=4, 4, 3)))
...
sp <- table(pp_data$age_category, pp_data$speaker_preference)
print(sp)
...
Both option are coded below. This also might be overkill...
