...
We predict that 4-year-old children will reliably provide logical reasoning for choosing a polite speaker over an impolite speaker (i.e. the proportion of children in the 4-year-old age category who indicate preference for a polite speaker and provide logical reasoning will be statistically significant).
...
```{r write and run simulation function}
...
We will measure preference between a polite speaker and an impolite speaker by presenting a forced-choice in which the speakers appear on opposite sides of the screen (counterbalanced) and the participant is asked, “Which friend do you want to play with?” (DV1). Pointing, reaching, and verbal answers will be coded equivalently as indication of preference. If a child does not answer, the child will be prompted three more times with analogous wording. If the child does not provide an answer, the session will be concluded.
...
We also intend to run three additional Bayesian binomial tests to determine whether the proportions of children in each age category who indicate preference for a polite speaker differ significantly from each other. 
...
binomial_test2vs4 <- bayes.binom.test(x=speaker_matrix["2","1"], n=speaker_matrix["2","0"]+speaker_matrix["2","1"], comp.theta=mean(pp_data$speaker_preference[pp_data$age_category==4]), cred.mass=0.95)
```

We will run four more Bayesian binomial tests to assess whether children exhibit shape, color, order, or side bias (i.e. whether the proportion of children overall who indicate preference for the triangle speaker, the square speaker, the red speaker, the yellow speaker, the first speaker, the second speaker, the speaker on the left of the forced-choice, or the speaker on the right of the forced-choice differs significantly from chance).
...
DV2_model <- stan_glm(filtered_reasoning~age, family=gaussian(), data=pp_data)
```

_Key_

- speaker_matrix: frequency matrix of speaker preference by age
- pp_data: Preferring Politeness dataframe
...
- binomial_test2v3: Bayesian binomial test comparing 2-year-old and 3-year-old age categories
- binomial_test3v4: Bayesian binomial test comparing 3-year-old and 4-year-old age categories
- binomial_test2v3: Bayesian binomial test comparing 2-year-old and 4-year-old age categories
- shape_matrix: frequency matrix of shape preference
- shape_preference: shape_preference (1 = triangle, 2 = square)
- color_matrix: frequency matrix of color preference
- color_preference: color preference (1 = red, 2 = yellow)
- order_matrix: frequency matrix of order preference
- order_preference: order preference (1 = first, 2 = second)
- side_matrix: frequency matrix of side preference
- side_preference: side preference (1 = left, 2 = right)
- binomial_test_shape: Bayesian binomial test for shape bias
- binomial_test_color: Bayesian binomial test for color bias
- binomial_test_order: Bayesian binomial test for order bias
- binomial_test_side: Bayesian binomial test for side bias
- DV1_model: Bayesian logistic regression model of speaker preference as a function of age
- age: age
- DV2_model: Bayesian logistic regression model of reasoning (inclusive of reasoning levels 1 - 3) as a function of age
- filtered_reasoning: reasoning filtered to include reasoning levels 1 - 3
